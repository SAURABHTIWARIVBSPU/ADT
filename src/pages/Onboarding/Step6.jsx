...
/* Form elements (same functionality, premium styles) */
...
/* ============ Data ============ */
const tones = ['Fun', 'Thrilling', 'Family-friendly', 'Luxury', 'Relaxed', 'Adventurous'];
...
function Step6() {
  ...
  const [tags, setTags] = useState(onboardingData.step6?.tags || '');
  const [tone, setTone] = useState(onboardingData.step6?.tone || '');
  const [language, setLanguage] = useState(onboardingData.step6?.language || '');
  const [story, setStory] = useState(onboardingData.step6?.story || '');
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!tags || !tone || !language || !story) {
      setError('Please fill all fields');
      return;
    }
    setError('');
    updateStep('step6', { tags, tone, language, story });
    navigate('/step-7');
  };
    ...
    <Container>
      ...
      <Header>
        <Brand>
          ...
          <StepBadge>Step 6 of 7 • Identity</StepBadge>
        </Brand>
      ...
      <Content>
        <Card>
          {/* LEFT: Form */}
          <Left>
            <Title>Adventure Identity & Story</Title>
            <Sub>Set tags/keywords, choose a tone, languages, and tell your story.</Sub>
            <RainbowBar />
            ...
            <div>
              ...
              <Input
                id="tags"
                type="text"
                placeholder="Comma-separated (e.g., sunrise trek, beginner-friendly, himalaya)"
                value={tags}
            ...
            <div style={{ marginTop: '0.8rem' }}>
              <FieldLabel htmlFor="lang">Language(s)</FieldLabel>
              <Input
                id="lang"
                type="text"
            ...
            <div style={{ marginTop: '0.8rem' }}>
              ...
              <TextArea
                id="story"
                placeholder="Why you started, what makes it memorable, your promise to guests..."
                value={story}
                onChange={e => setStory(e.target.value)}
                rows={8}
              />
              <Counter>{story.length} characters</Counter>
            </div>

            {error && <ErrorText role="alert">{error}</ErrorText>}

            <Row>
              <ButtonGhost onClick={() => { setTags(''); setTone(''); setLanguage(''); setStory(''); }}>
                Clear
              </ButtonGhost>
          ...
          {/* RIGHT: Tips */}
          <Right>
            <Sub>Writing Tips</Sub>
            <Tips>
              <ul style={{ margin: 0, paddingLeft: '1.1rem', lineHeight: 1.55 }}>
                <li><strong>Lead with the hook:</strong> one-liner that captures the experience.</li>
                <li><strong>Voice = Tone:</strong> keep your word choices aligned with the tone you select.</li>
                <li><strong>Clarity over fluff:</strong> mention difficulty, duration, and highlight moments.</li>
                <li><strong>Inclusive language:</strong> specify languages to welcome more guests.</li>
                <li><strong>Keywords:</strong> add 3–7 relevant tags to improve discovery.</li>
              </ul>
            </Tips>

            <Sub style={{ marginTop: '1rem' }}>Live Preview</Sub>
            <Tips as="div">
              <div style={{ fontWeight: 800, color: '#e6f1ff' }}>
                {tone || 'Your tone'}, {language || 'Language(s)'}
              </div>
              <div style={{ marginTop: '.35rem', color: '#cfe8ff' }}>
                Tags: <span style={{ color: tags ? '#00ffa3' : '#9fb3d9' }}>
                  {tags || '—'}
                </span>
              </div>
              <div style={{ marginTop: '.6rem', color: '#cfe8ff' }}>
                {story ? story : 'Your story preview will appear here…'}
              </div>
            </Tips>
          </Right>
        </Card>
